...
def load_stage2_model(checkpoint_path, device='cuda'):
    ...
    print(f"   ✅ Stage 2 checkpoint loaded successfully")
    llm.eval()
...
def set_routing_mask(llm, vision_expert_id, text_expert_id, num_visual_tokens, num_text_tokens, device='cuda'):
    """
...
def compute_loss_single_example(
    clip_model, vision_connector, llm,
    pixel_values, input_ids, vision_expert_id, text_expert_id, device='cuda'
):
    """
    ...
    with torch.no_grad():
        ...
        pixel_values = pixel_values.unsqueeze(0).to(device) if pixel_values.dim() == 3 else pixel_values.to(device)
...
def run_routing_ablation(
    ...
    for i, batch in enumerate(tqdm(val_loader, total=num_samples)):
        ...
        loss_normal = compute_loss_single_example(
            ...
            vision_expert_id=0, text_expert_id=1,
            device=device
        )
        ...
        loss_flipped = compute_loss_single_example(
            clip_model, vision_connector, llm,
            pixel_values, input_ids,
            vision_expert_id=1, text_expert_id=0,
            device=device
        )
...
def create_visualization(normal_losses, flipped_losses, output_dir):
    ...
    ax.set_title('Expert Routing Swap Study',

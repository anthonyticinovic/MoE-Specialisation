...
class AttentionRoutingAnalyzer(CrossModalityPurityAnalyzer):
    ...
    def extract_random_samples(
        ...
        # Random sample
        np.random.seed(seed)
        selected_indices = np.random.choice(len(valid_samples), size=min(num_samples, len(valid_samples)), replace=False)
    ...
    def _compute_token_level_attention_by_expert(
        ...
        return {
            'expert0_attentions': expert0_attentions,
            'expert1_attentions': expert1_attentions
        }
    ...
    def analyze_attention_routing_across_layers(
        ...
        """
        print(f"\n{'='*70}")
        print(f"🔬 Analyzing Attention-Routing Patterns Across All Layers")
        print(f"{'='*70}")
        ...
        layer_token_data = {
            ...
            for layer_idx in range(num_layers)
        } if analyze_expert_correlation else None
    ...
    def _plot_attention_routing_evolution(
        ...
        ax.plot(layers, means_t2v, linewidth=2.5, label='Text → Vision (cross-modal)', color='#1f77b4', linestyle='-')
        ...
        ax.plot(layers, means_t2t, linewidth=2, label='Text → Text (intra-modal)', color='#ff7f0e', linestyle='--')
        ...
        ax.text(0.98, 0.02, 'Note: Intra-modal excludes self-attention', 
        ...
        ax.fill_between(layers, means_text_ent - stds_text_ent, means_text_ent + stds_text_ent, alpha=0.3, color='#d62728')
        
        ax.plot(layers, means_vision_ent, linewidth=2, label='Vision Attention Entropy', color='#9467bd')
        ax.fill_between(layers, means_vision_ent - stds_vision_ent, means_vision_ent + stds_vision_ent, alpha=0.3, color='#9467bd')
    ...
    def _plot_expert_attention_correlation(
        self,
        layer_token_data: Dict[int, Dict[str, List[float]]],
        output_dir: str
    ):
        """
        ...
        plot_continuous_segments(layers, expert0_means, expert0_stds, 
                                '#1f77b4', '-', 'Expert 0')
        
        # Plot Expert 1 (red, dashed)
        plot_continuous_segments(layers, expert1_means, expert1_stds,
                                '#d62728', '--', 'Expert 1')
        ...
        ax.text(0.98, 0.02, 'Note: Token-level analysis. Each point = one text token\'s attention to vision.', 
                transform=ax.transAxes, fontsize=9, verticalalignment='bottom',
                horizontalalignment='right', alpha=0.7, style='italic')
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to attention routing analysis config JSON"
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    # Print configuration
    print(f"\n{'='*70}")
    print(f"🔬 MoE Attention-Routing Analysis")
    print(f"{'='*70}")
    ...
    print(f"\n{'='*70}")
    print(f"✅ Attention-routing analysis complete!")

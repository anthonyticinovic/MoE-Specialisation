...
def create_comparison_table(stage2_metrics: Dict, stage3_metrics: Dict, difficulties: List[str]) -> str:
    ...
    for difficulty in difficulties:
        ...
        lines.append("-" * 100)
        lines.append(f"{'Metric':<20} {'Stage 2':>15} {'Stage 3':>15} {'Δ (S3-S2)':>15} {'Winner':>15}")
        lines.append("-" * 100)
        ...
        lines.append("-" * 100)
        lines.append(f"Hallucinations (FP): {s2['false_positive']:>7d}        {s3['false_positive']:>7d}        {s3['false_positive']-s2['false_positive']:>+7d}")
    ...
    for difficulty in difficulties:
        ...
        lines.append(f"• {difficulty.capitalize()} - Accuracy: {acc_winner} wins by {acc_margin:.2f}%")
        lines.append(f"• {difficulty.capitalize()} - F1 Score: {f1_winner} wins by {f1_margin:.2f}%")
        lines.append(f"• {difficulty.capitalize()} - Hallucinations: Stage 3 has {abs(halluc_change)} {halluc_direction} ({halluc_change:+d})")

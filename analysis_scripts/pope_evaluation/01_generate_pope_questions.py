...
def generate_pope_questions_for_image(
    ...
    if len(cooccurring_not_in_image) >= num_questions:
        ...
        negative_adversarial = random.sample(objects_not_in_image, min(num_questions, len(objects_not_in_image)))
...
def generate_pope_dataset(
    ...
    print(f"   Questions per image: {questions_per_image} positive + {questions_per_image} negative = {questions_per_image * 2} per difficulty")
    ...
    print(f"   Building co-occurrence matrix...")
    ...
    for difficulty in ['random', 'popular', 'adversarial']:
        ...
        print(f"   {difficulty.capitalize():12s}: {len(all_questions[difficulty])} questions ({num_yes} yes, {num_no} no)")

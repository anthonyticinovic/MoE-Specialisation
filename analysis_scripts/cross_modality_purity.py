...
class SyntheticImageGenerator:
    ...
    def generate_color_patch(self, color: str) -> Image.Image:
        ...
        if color not in self.colors:
            raise ValueError(
                f"Unknown color: {color}. Available: {list(self.colors.keys())}"
            )
...
class CrossModalityPurityAnalyzer:
    ...
    def analyze_representation(
        self, concept: str, expert: str, layer: int, modality: str
    ) -> np.ndarray:
        """
        ...
        if modality not in ["vision", "text"]:
            raise ValueError(
                f"Invalid modality: {modality}. Must be 'vision' or 'text'."
            )
        if not (-2 <= layer < 32):
        ...
        if layer in [-2, -1]:
            if expert == "vision":
                if layer == -2:
                    embeddings = self._prepare_vision_input(
                        concept, use_connector=False
                    )
                else:  # layer == -1
            ...
            routing_mask = self._force_routing_through_expert(
                expert_id, batch_size, num_tokens
            )
    ...
    def analyze_vocab(
        self, concept: str, expert: str, layer: int, modality: str, top_k: int = 10
    ) -> Dict[str, float]:
        """
    ...
    def compute_cosine_similarity(
        self, concept: str, layer: int, modality: str = "vision"
    ) -> float:
        """
        Compute cosine similarity between vision and text expert representations.

        Note: Each expert uses its native modality regardless of the modality arg:
            - vision expert -> vision inputs
            - text expert   -> text inputs
        The modality parameter is kept for backward compatibility but is ignored.

        Args:
            concept: Concept to analyze
            layer: Layer index
            modality: Unused (kept for compatibility)

        Returns:
            Cosine similarity score [-1, 1]
        """
        # Get representations from both experts on their native modalities
        vision_rep = self.analyze_representation(
            concept, "vision", layer, modality="vision"
        )
        text_rep = self.analyze_representation(concept, "text", layer, modality="text")
    ...
    def compute_probability_ratio(
        ...
        """
        # Get vocab predictions from both experts on their native modalities
        vision_vocab = self.analyze_vocab(
            concept, "vision", layer, modality="vision", top_k=50
        )
        text_vocab = self.analyze_vocab(
            concept, "text", layer, modality="text", top_k=50
        )
        ...
        return {"vision_prob": vision_prob, "text_prob": text_prob, "ratio": ratio}
        ...
        for concept in concepts:
            ...
            for layer in layers:
                ...
                try:
                    ...
                    text_vocab = self.analyze_vocab(
                        concept, "text", layer, "text", top_k=10
                    )
    ...
    def _visualize_results(self, results: Dict, output_dir: str):
        ...
        for concept in concepts:
            ...
            plt.title(
                f"Modality Purity for '{concept}'\n(Ratio closer to 0 = more purity)",
                fontweight="bold",
            )
            plt.legend()

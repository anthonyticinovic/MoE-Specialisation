...
def evaluate_response_simple(response, reference=None):
    ...
    content_words = ['shows', 'depicts', 'features', 'contains', 'includes', 
                     'appears', 'looks', 'seems', 'probably', 'likely',
                     'color', 'large', 'small', 'red', 'blue', 'white', 'black']
    has_content = any(word in response.lower() for word in content_words)
    if has_content:
        score += 15
    
    # Generic template penalty
    generic_starts = ['the image shows', 'the image depicts', 'this image shows',
...
def load_llava_subset(json_path, num_samples=100, seed=42):
    ...
    for item in data:
        if 'conversations' in item and len(item['conversations']) >= 2 and 'image' in item:
            ...
            question = item['conversations'][0]['value'].replace('<image>', '').replace('\n', ' ').strip()
...
def main():
    ...
    print(f"🔮 Generating conversational responses...")

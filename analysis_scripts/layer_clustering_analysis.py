...
class MoEClusteringAnalyzer(CrossModalityPurityAnalyzer):
        ...
        for concept, samples in concept_samples.items():
            ...
            for sample in samples:
                ...
                for layer_idx in target_layers:
                    ...
                    combined_hidden = combined_layer_states[layer_idx + 1]  # [1, 257+text_len, hidden_dim]
                    
                    # Split combined hidden states back into vision and text
                    vision_hidden = combined_hidden[:, :num_vision_tokens, :]  # [1, 257, hidden_dim]
                    text_hidden = combined_hidden[:, num_vision_tokens:, :]  # [1, text_len, hidden_dim]
                    ...
                    if hasattr(moe_layer, '_last_router_logits'):
                        ...
                        all_router_logits = moe_layer._last_router_logits  # [batch, total_seq_len, num_experts]
                        
                        # Debug: Print shape and sample values for first sample only
                        if sample_count == 1 and layer_idx == target_layers[0]:
                            print(f"      DEBUG: all_router_logits shape: {all_router_logits.shape}")
                            print(f"      DEBUG: num_vision_tokens: {num_vision_tokens}")
                            print(f"      DEBUG: Vision router logits sample (first 3 tokens): {all_router_logits[0, :3, :]}")
                            print(f"      DEBUG: Text router logits sample (first 3 tokens): {all_router_logits[0, num_vision_tokens:num_vision_tokens+3, :]}")
                        
                        vision_router_logits = all_router_logits[:, :num_vision_tokens, :]  # [1, 257, 2]
                        text_router_logits = all_router_logits[:, num_vision_tokens:, :]  # [1, text_len, 2]
                        
                        # Convert to probabilities
                        vision_router_probs = torch.softmax(vision_router_logits[0], dim=-1)  # [257, 2]
                        text_router_probs = torch.softmax(text_router_logits[0], dim=-1)  # [text_len, 2]
                        
                        # Debug: Print probability distributions
                        if sample_count == 1 and layer_idx == target_layers[0]:
                            print(f"      DEBUG: Vision probs sample (first 3 tokens): {vision_router_probs[:3, :]}")
                            print(f"      DEBUG: Text probs sample (first 3 tokens): {text_router_probs[:3, :]}")
                            vision_expert0_count = (vision_router_probs.argmax(dim=1) == 0).sum().item()
                            text_expert0_count = (text_router_probs.argmax(dim=1) == 0).sum().item()
                            print(f"      DEBUG: Vision tokens → Expert 0: {vision_expert0_count}/{len(vision_router_probs)}")
                            print(f"      DEBUG: Text tokens → Expert 0: {text_expert0_count}/{len(text_router_probs)}")
    ...
    def plot_clustering_analysis(
        ...
        modality_colors = {'vision': '#1f77b4', 'text': '#ff7f0e'}  # Blue for vision, orange for text
    ...
    def generate_clustering_report(
        ...
        with open(report_path, 'w') as f:
            ...
            f.write("| Label Type | Silhouette Score | Davies-Bouldin Index | # Clusters | # Samples |\n")
            f.write("|------------|------------------|----------------------|------------|-----------|\n")
            
            for label_type in ['concept', 'modality', 'expert']:
                m = metrics[label_type]
                f.write(f"| {label_type.capitalize()} | {m['silhouette_score']:.4f} | {m['davies_bouldin_index']:.4f} | {m['n_clusters']} | {m['n_samples']} |\n")
            
            f.write("\n")
            
            # Metric interpretation
            f.write("### Metric Interpretation\n\n")
            f.write("- **Silhouette Score**: Measures how well samples are clustered (-1 to 1, higher is better)\n")
            ...
            if 'Expert 0' in contingency.columns and 'Expert 1' in contingency.columns:
                ...
                if vision_to_0 > 70 and text_to_1 > 70:
                    f.write("**Strong modality specialization detected**: Vision → Expert 0, Text → Expert 1\n")
                elif vision_to_1 > 70 and text_to_0 > 70:
                    f.write("**Strong modality specialization detected**: Vision → Expert 1, Text → Expert 0\n")
                elif max(vision_to_0, vision_to_1) < 60 or max(text_to_0, text_to_1) < 60:
                    f.write("**Weak modality specialization**: Routing is relatively balanced across experts\n")
                else:
                    f.write("**Moderate modality specialization**: Some preference but not strongly separated\n")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to clustering analysis config JSON"
    )
    parser.add_argument(
        "--layers",
        nargs="+",
    ...
    # Print configuration
    print(f"\n{'='*70}")
    print(f"🔬 MoE Layer Clustering Analysis")
    print(f"{'='*70}")
    ...
    for layer_idx in config['analysis']['target_layers']:
        ...
        for label_type in ['concept', 'modality', 'expert']:
            ...
            print(f"   {label_type.capitalize()} clustering - Silhouette: {metrics[label_type]['silhouette_score']:.4f}, Davies-Bouldin: {metrics[label_type]['davies_bouldin_index']:.4f}")
        ...
        if config['output'].get('save_representations', False):
            repr_path = os.path.join(
                ...
                f"layer_{layer_idx}_representations.npz"
            )
            np.savez(
                repr_path,
